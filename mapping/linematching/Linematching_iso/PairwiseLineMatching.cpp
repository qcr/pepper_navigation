...
void inline normalize(std::vector<double> &vect)
{
    double v_norm2 = 0.0;
    for(std::vector<double>::iterator it=vect.begin();it != vect.end();it++)
        v_norm2 += (*it)*(*it);
     v_norm2 = std::sqrt(v_norm2);

     for(std::vector<double>::iterator it=vect.begin();it != vect.end();it++)
         (*it)/=v_norm2;
}

double inline normL2(std::vector<double> &vect)
{
    double v_norm2 = 0.0;
    for(std::vector<double>::iterator it=vect.begin();it != vect.end();it++)
        v_norm2 += (*it)*(*it);

   return(std::sqrt(v_norm2));
...
double PairwiseLineMatching::GlobalRotationOfImagePair_(ScaleLines &linesInLeft, ScaleLines &linesInRight)
{
  ...
  std::vector<double> angleHistLeft(dim,0.0);
  std::vector<double> angleHistRight(dim,0.0);
  std::vector<double> lengthLeft(dim,0.0);//lengthLeft[i] store the total line length of all the lines in the ith angle bin.
  std::vector<double> lengthRight(dim,0.0);
 ...
 normalize(angleHistLeft);
 normalize(angleHistRight);
 normalize(lengthLeft);
 normalize(lengthRight);



  //step 2: find shift to decide the approximate global rotation
   std::vector<double> difVec(dim,0.0);
 // BIAS::Vector<double> difVec(dim);//the difference vector between left histogram and shifted right histogram
  double minDif=10;//the minimal angle histogram difference
  double secondMinDif = 10;//the second minimal histogram difference
  unsigned int minShift=10;//the shift of right angle histogram when minimal difference achieved
  unsigned int secondMinShift=10;//the shift of right angle histogram when second minimal difference achieved

  std::vector<double> lengthDifVec(dim,0.0);
  //BIAS::Vector<double> lengthDifVec(dim);//the length difference vector between left and right
  double minLenDif = 10;//the minimal length difference
  double secondMinLenDif = 10;//the second minimal length difference
  unsigned int minLenShift=10;//the shift of right length vector when minimal length difference achieved
  unsigned int secondMinLenShift=10;//the shift of right length vector when the second minimal length difference achieved
 ...
 // std::cout<<"minimal histgram distance = "<<minDif<<", Approximate global rotation angle = "<<rotationAngle<<std::endl;
...
void PairwiseLineMatching::BuildAdjacencyMatrix_(ScaleLines &linesInLeft,ScaleLines &linesInRight)
{
  ...
  //store the descriptor distance of lines in left and right images.
    Matrix<double> desDisMat(numLineLeft,numLineRight);
...
//	desDisMat.Save("DescriptorDis.txt");
    ...
    std::vector<double> adjacenceVec(dim*(dim+1)/2,0.0);
    ...
    Matrix<double>       projRatioRight(numLineRight,numLineRight);
  ...
  //  std::cout<<"Number of 'converged' eigenvalues  : " << dprob.ConvergedEigenvalues() <<std::endl;
	int k = 1;
	if (dprob.ConvergedEigenvalues()==1) k=0;
	 if (dprob.ConvergedEigenvalues()==0) throw(-1);
  ...
  if(dprob.EigenvectorsFound()){
  	...
  	for(unsigned int j=0; j<dim; j++){
  		...
  		value = fabs(dprob.Eigenvector(k,j));
  		meanEigenVec += value;
		...
		std::vector<unsigned int > &matchResult)
{
  ...
  */
	...
	while(1){
		...
		//	cout<<"ct = "<<ct<<" "<<eigenMap_.empty()<<eigenMap_.size()<<endl;
			//if(ct>=eigenMap_.size()){
				//if(ct ==1)
				//    flag =1;
				
				//break;}
			//else{
		    //cout<<"cp3xx = "<<ct<<" => "<<eigenMap_.size()<<endl;
			  iter++;
			  if(iter==eigenMap_.end())break;
    ...
    //std::cout<<"matchRet1.size= "<<matchRet1.size()<<", minOfEigenVec_= "<<minOfEigenVec_<<std::endl;
}
