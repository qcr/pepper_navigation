...
//#define DEBUGLinesInOctaveImages 1
...
//	cout<<"LineDescriptor object is constructed"<<endl;
    LowestThreshold = 0.35;//2 is used to show recall ratio;  0.2 is used to show scale space results, 0.35 is used when verify geometric constraints.
	NNDRThreshold   = 0.6;
   bDistThreshold = 32;
     bdm = cv::line_descriptor::BinaryDescriptorMatcher::createBinaryDescriptorMatcher();
}
...
/*	BIAS::Image<float> img;
	...
	float factor = sqrt(2.0);//the down sample factor between connective two octave images
...
/*					rho2 = scale[octaveID] * fabs(edLineVec_[octaveID]->lineEquations_[lineIDInOctave][2]);
					nears = fabs(rho1 - rho2);
					if(nears>nearThreshold){
...
int LineDescriptor::OctaveKeyLines(cv::Mat & image, ScaleLines &keyLines)
{
 ...
 /*BIAS::Image<unsigned char> curim;
 curim.Init(image.cols, image.rows,1);
 memcpy(curim.GetImageData(),image.data, curim.GetSizeByte());
    BIAS::Image<float> img;
    BIAS::ImageConvert::ConvertST(curim,img, BIAS::ImageBase::ST_float);//copy the input image
*/
    float preSigma2 = 0;//orignal image is not blurred, has zero sigma;
    float curSigma2 = 1.0;//[sqrt(2)]^0=1;
    float factor = sqrt(2.0);//the down sample factor between connective two octave images
   // BIAS::Gauss<float, float> theGauss;
    ...
    for(unsigned int octaveCount = 0; octaveCount<numOfOctave_; octaveCount++){
      ...
      //  BIAS::Image<float> blur;
         ...
         * curSigma = [sqrt(2)]^octaveCount;
         * increaseSigma^2 = curSigma^2 - preSigma^2 */
        double increaseSigma = sqrt(curSigma2-preSigma2);
      /*  theGauss.SetSigma(increaseSigma);
        switch(ksize_){
        case 3: theGauss.Filter3x3Grey(img, blur); break;
        case 5: theGauss.Filter5x5Grey(img, blur); break;
        ...
        switch(ksize_){
            case 3: cv::GaussianBlur(img, blur, cv::Size(3,3), increaseSigma); break;
            case 5: cv::GaussianBlur(img, blur, cv::Size(5,5), increaseSigma); break;
            case 7: cv::GaussianBlur(img, blur, cv::Size(7,7), increaseSigma); break;
            case 9: cv::GaussianBlur(img, blur, cv::Size(9,9), increaseSigma); break;
            case 11: cv::GaussianBlur(img, blur, cv::Size(11,11), increaseSigma); break;
            default: cv::GaussianBlur(img, blur, cv::Size(5,5), increaseSigma); break;
...
//		}
        ...
        /*img.Init((int)((float) blur.GetWidth() / factor), (int)((float) blur.GetHeight() / factor),1);
        sample(blur.GetImageData(),img.GetImageData(), factor, blur.GetWidth(),  blur.GetHeight());*/
        img.create((int)(blur.rows/factor), (int)(blur.cols/factor), CV_8U);
     //   img.create((int)(blur.rows/factor), (int)(blur.cols/factor), CV_32F);

        sample(blur.data,img.data, factor, blur.cols,  blur.rows);
...
void LineDescriptor::findLineDesc(ScaleLines &keyLines)
{
...
/*
	for(int i = 0; i<keyLines.size();i++)
		if(keyLines[i][0].lineLength>50)
			temp.push_back(keyLines.at(i));
	keyLines = temp;
	*/
//	BIAS::TimeMeasure timer;
//	timer.Start();
...
//	timer.Print();
//	timer.Reset();
    computeBinaryLineDesc(keyLines);
    return 1;
}
...
void LineDescriptor::computeBinaryLineDesc(ScaleLines &keyLines){
 OctaveSingleLine *pSingleLine;
 float *desVec;
 unsigned char *bdesVec;
    for(unsigned int linenum=0;linenum<keyLines.size();linenum++){
        pSingleLine = &(keyLines[linenum][0]);
        pSingleLine->bdescriptor.resize(32);
        desVec = pSingleLine->descriptor.data();
        bdesVec = pSingleLine->bdescriptor.data();
        for ( int comb = 0; comb < 32; comb++ ){
             bdesVec[comb] = binaryConversion( &desVec[8 * combinations[comb][0]], &desVec[8 * combinations[comb][1]]);
    }
...
/*void LineDescriptor::computeBinaryLineDescFull(ScaleLines &keyLines){
    int sz = numOfBand_*(numOfBand_-1)/2;
    OctaveSingleLine *pSingleLine;
    float *desVec;
    unsigned char *bdesVec;
    ...
    for(unsigned int linenum=0;linenum<keyLines.size();linenum++){
        pSingleLine = &(keyLines[linenum][0]);
        pSingleLine->bdescriptor.resize(sz);
        desVec = pSingleLine->descriptor.data();
        bdesVec = pSingleLine->bdescriptor.data();

          int comb =0;
           for ( int comb1 = 0; comb1 < numOfBand_; comb1++ ){
               for ( int comb2 = 1+comb1; comb2 < numOfBand_; comb2++ )
           {

               keyLines[linenum][0].bdescriptor.at(comb) = binaryConversion( &desVec[8 * comb1], &desVec[8 * comb2]);
               comb++;
        }
           }
...
int LineDescriptor::LineMatchingBinary(ScaleLines &keyLinesLeft, ScaleLines &keyLinesRight,std::vector<unsigned int> &matchResult){
cv::Mat desleft, desright;
OctaveSingleLine *pSingleLine;
unsigned char *bdesVec;

pSingleLine = &(keyLinesLeft[0][0]);
int numdesc =  pSingleLine->bdescriptor.size();
int nLines = keyLinesLeft.size();

desleft.create(nLines,numdesc,CV_8UC1);

for(int linenum=0;linenum<nLines;linenum++){
     pSingleLine = &(keyLinesLeft[linenum][0]);
     bdesVec = pSingleLine->bdescriptor.data();
    for(int j=0;j<numdesc;j++){
        desleft.at<uchar>(linenum,j) = bdesVec[j];
              }
        }


pSingleLine = &(keyLinesRight[0][0]);
numdesc =  pSingleLine->bdescriptor.size();
nLines = keyLinesRight.size();

desright.create(nLines,numdesc,CV_8UC1);

for(int linenum=0;linenum<nLines;linenum++){
     pSingleLine = &(keyLinesRight[linenum][0]);
     bdesVec = pSingleLine->bdescriptor.data();
    for(int j=0;j<numdesc;j++){
        desright.at<uchar>(linenum,j) = bdesVec[j];
              }
        }
  std::vector<cv::DMatch> matches;
  bdm->match(desleft, desright, matches);
 int nmatches = matches.size();
 matchResult.clear();
  for(int i=0;i<nmatches;i++){
      if(matches.at(i).distance<bDistThreshold){
          matchResult.push_back(matches.at(i).trainIdx);
          matchResult.push_back(matches.at(i).queryIdx);
      }
}
