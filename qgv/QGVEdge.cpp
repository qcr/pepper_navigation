...
void QGVEdge::setWeight(const double &wt){
   setAttribute("len", QString::number(wt));
}
...
void QGVEdge::updateLayout()
{
    ...
    {
        if(spl->list->eflag & spl->list->sflag)
        {
            if(spl->list->sp.y==spl->list->ep.y){
                _head_arrow = toArrow(QLineF(QGVCore::toPoint(spl->list->list[spl->list->size-1], gheight), QGVCore::toPoint(spl->list->sp, gheight)));
                _tail_arrow = toArrow(QLineF(QGVCore::toPoint(spl->list->list[0], gheight), QGVCore::toPoint(spl->list->ep, gheight)));
            }
