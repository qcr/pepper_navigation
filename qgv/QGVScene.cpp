...
QGVEdge *QGVScene::addEdge(QGVNode *source, QGVNode *target, const double wt)
{
    Agedge_t* edge = agedge(_graph->graph(), source->_node->node(), target->_node->node(), NULL, TRUE);
    if(edge == NULL)
    {
        qWarning()<<"Invalid egde :";
        return 0;
    }
...
void QGVScene::readLayout(const QString &text)
{

    _graph->setGraph(QGVCore::readfromfile(text.toLocal8Bit().constData()));
